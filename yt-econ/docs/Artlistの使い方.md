# 素材（動く背景・B-roll・BGM・効果音）の集め方

**何もしなくても最初から動きます。** 動画を作るとき素材が無ければ、ログイン不要の
フリー素材サイト Mixkit（商用可・クレジット不要）から動画 27 本・曲 4 曲・効果音 8 個を
自動で集めます（`python -m ytecon.freeassets`）。Artlist はその上に、より良い素材を
足したいときの任意の手段です。以下は Artlist を使う場合の手順。
...
`ytecon_mac/素材をそろえる.command` をダブルクリックすると、次を続けてやります。

1. Artlist を専用のブラウザ窓で開く（初回だけログイン。次回から不要）
2. 下の一覧の検索語を順に開き、ダウンロードを押す（**自動**）。
   押せなかった検索語は、その画面で**あなたが**好きな素材のダウンロードを押す（**手伝い**）。
   どちらでも、ファイル名・タグ・フォルダ分けは自動
3. `~/Downloads` に落ちている立ち絵 zip や Artlist のファイルも `assets/` に振り分ける

お試しは `.venv/bin/python artlist_fetch.py --limit 5` のように少なく始めてください。
自動で押す部分は開発側から Artlist に届かないため未検証です。押せなかったときは
`~/ytecon_artlist_debug/` に画面写真とボタン候補の一覧が残るので、それを送ってもらえれば直します。
自動化を避けたいときは `--manual`（押すのは自分、整理だけ道具）。

> Artlist の規約は自動化ツールでの取得を制限しています。この道具は人が押すのと同じ操作を
> 人と同じ速さで行い、まとめ落としはしませんが、気になる場合は `--manual` を使ってください。
