...
- **{student_tag} は各セクションに 1 回、少しズレた解釈（ボケ）を言う。** 新卒らしい早とちりや極端な結論
  （「じゃあ値上げした会社は全部悪なのだ」「つまり給料は上がらないほうがいいのだ？」）を短く言い、
  {teacher_tag} がやさしく、しかし正確に直す。笑いより「よくある勘違い」を先回りして潰すのが目的
- **{teacher_tag} の相づちだけの文（「ええ。」「そうね。」）を単独の文にしない。** 次の文につなげる
  （悪い例:「ええ。」「では、なぜ消えるのか。」→ 良い例:「ええ、では、なぜ消えるのかしら。」）。
  合成音声は 1〜2 語の文を不自然に読む
- 「2 つのレンズで見る」のような枠組みは、promise で「レンズ1は〜」「レンズ2は〜」と**番号つきで別の文**にし、
  本編でもそのレンズに入るときに「ここからレンズ1よ」と番号で呼ぶ（画面の箇条書きと対応させる）
...
_BAD_NOTE = re.compile(r"台本|描写|例示|仮の|一般的な期待|イメージ|想定|^概念図$|概念図（|模式図")
# 仮の数字を数字カードにしない
